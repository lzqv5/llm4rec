...
def build_chat_prompt(prompt_idx=0):
    ...
    if prompt_idx == 0:
        ...
        human_message_prompt += """to be implemented(#1)
        你的第一个回复应该只是“已理解”。
        """
    else:
        human_message_prompt += """to be implemented(#2)
        你的第一个回复应该只是“已理解”。
        """
...
def rebuild_output(mean_score, outputs):
    flag = 1e3
    idx = 0
...
def single_session_processing(generation: list, output_parser: JsonExtractor):
    ...
    return rebuild_output(mean_score, outputs)
...
def resorting(jd, cv, candidates, chat_model, chat_prompt, output_parser,  rate_limit_per_min=3500, tokens_per_min=90000, token_per_req=4000, delta=2.5):
    if not isinstance(chat_prompt, list):
        ...
        outputs_new = [rebuild_output(mean_score=mean_scores[j], outputs=[outputs[i][j] for i in range(prompt_nums)]) for j in range(candidates_nums)]
